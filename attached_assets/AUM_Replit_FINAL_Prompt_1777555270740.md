...
```javascript
const CLOUD = 'https://res.cloudinary.com/diujqvfed/image/upload';
...
Title: "THE MANDATE" 16px SemiBold charcoal.
...
Bottom: Full-width charcoal "BEGIN THE MANDATE →" button.
...
| "Next level"       | "Advancing to the next phase of the Mandate."    |
| "Score"            | "Trust Balance"                                    |
| "Points"           | "Capital"                                          |
...
## AUM LOGO SPECIFICATION

The AUM logo is a 4-bar ascending mark alongside the "AUM" wordmark in Times New Roman Bold crimson.

```svg
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 600 200" width="600" height="200">
  <rect width="600" height="200" fill="#FFFFFF"/>
  <!-- Bar 1: Light grey — shortest -->
  <rect x="20" y="121" width="11" height="34" fill="#BBBBBB"/>
  <!-- Bar 2: Charcoal — medium -->
  <rect x="36" y="99" width="11" height="56" fill="#444444"/>
  <!-- Bar 3: Black — tall -->
  <rect x="52" y="85" width="11" height="70" fill="#111111"/>
  <!-- Bar 4: Red — tallest, top = AUM cap height -->
  <rect x="68" y="69" width="11" height="86" fill="#CC0000"/>
  <!-- AUM wordmark -->
  <text x="90" y="155"
    font-family="'Times New Roman', Times, serif"
    font-weight="700" font-size="120" fill="#CC0000">AUM</text>
</svg>
```

Save this as `/assets/aum_logo.svg` in the project.

**Logo usage in app:**
- Login screen: centered, width 200px
- Home screen header: width 80px, top left
- Splash screen: centered, width 280px, white background
- All other screens: width 60px in header bar

**In-app header text version (fallback if SVG not rendering):**
```javascript
<Text style={{ fontFamily: 'serif', fontWeight: '700',
  fontSize: 22, color: '#CC0000', letterSpacing: 1 }}>AUM</Text>
<Text style={{ fontSize: 8, color: '#CC0000',
  letterSpacing: 3, textTransform: 'uppercase' }}>THE WEALTH GAME</Text>
```
...
All character and Titan photos are hosted on Cloudinary with automatic face-detection circular cropping.

**URL format:**
```javascript
const CLOUDINARY_CLOUD = process.env.CLOUDINARY_CLOUD_NAME;
const photo = (publicId) =>
  `https://res.cloudinary.com/diujqvfed/image/upload/w_200,h_200,c_fill,g_face,r_max/${publicId}`;
```

Store `CLOUDINARY_CLOUD_NAME` in Replit Secrets (never hardcode).

**Character photo public IDs:**
```javascript
const CHARACTER_PHOTOS = {
  arthur_sterling:  'aum/arthur_sterling',
  barnaby_buckley:  'aum/barnaby_buckley',
  victor_crane:     'aum/victor_crane',
};
```

**Titan photo public IDs (one per day):**
```javascript
const TITAN_PHOTOS = {
  'Ray Dalio':              'aum/titans/ray_dalio',
  'Janet Yellen':           'aum/titans/janet_yellen',
  'Paul Volcker':           'aum/titans/paul_volcker',
  'George Soros':           'aum/titans/george_soros',
  'Mohamed El-Erian':       'aum/titans/mohamed_el_erian',
  'Nouriel Roubini':        'aum/titans/nouriel_roubini',
  'Carmen Reinhart':        'aum/titans/carmen_reinhart',
  'Stanley Druckenmiller':  'aum/titans/stanley_druckenmiller',
  'Ben Bernanke':           'aum/titans/ben_bernanke',
  'Raghuram Rajan':         'aum/titans/raghuram_rajan',
  'Howard Marks':           'aum/titans/howard_marks',
  'Grantham Jeremy':        'aum/titans/jeremy_grantham',
  'Peter Lynch':            'aum/titans/peter_lynch',
  'John Templeton':         'aum/titans/john_templeton',
  'William Bernstein':      'aum/titans/william_bernstein',
  'Seth Klarman':           'aum/titans/seth_klarman',
  'David Swensen':          'aum/titans/david_swensen',
  'Joel Greenblatt':        'aum/titans/joel_greenblatt',
  'Cliff Asness':           'aum/titans/cliff_asness',
  'Michael Burry':          'aum/titans/michael_burry',
  'Harry Markowitz':        'aum/titans/harry_markowitz',
  'William Sharpe':         'aum/titans/william_sharpe',
  'Eugene Fama':            'aum/titans/eugene_fama',
  'Daniel Kahneman':        'aum/titans/daniel_kahneman',
  'Gary Brinson':           'aum/titans/gary_brinson',
  'Martin Leibowitz':       'aum/titans/martin_leibowitz',
  'Philippe Jorion':        'aum/titans/philippe_jorion',
  'Nassim Taleb':           'aum/titans/nassim_taleb',
  'Keith Ambachtsheer':     'aum/titans/keith_ambachtsheer',
  'Warren Buffett':         'aum/titans/warren_buffett',
  'John Bogle':             'aum/titans/john_bogle',
};
```
...
const FallbackAvatar = ({ name }) => (
  <View style={{
    ...
    <Text style={{ color: colors.charcoal, fontSize: 20, fontWeight: '500' }}>
      {name?.charAt(0) || '?'}
    </Text>
  </View>
);
...
The Cloudinary cloud name must be set as a Replit Secret named `CLOUDINARY_CLOUD_NAME` before running the app. Photos will display as initials fallback avatars until the cloud name is configured. This is expected behaviour during development.
